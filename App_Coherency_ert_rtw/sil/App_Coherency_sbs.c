 ...
 * Component: App_Coherency
 * Component Simulink Path: App_Coherency
 * Simulation Mode: SIL
 *
 */

#define S_FUNCTION_NAME                App_Coherency_sbs
...
static boolean_T startAndSetupApplication(SimStruct *S)
{
...

  {
    ...
    {
      mxArray * codeGenComponent = mxCreateString(
        "Position_Sensor_Characteristics");
      if (coderAssumpHostAppSvcCreate(&pCoderAssumptionsApp, pComms, pXILUtils,
           0, 1, codeGenComponent, 1, 0, 1) != CODERASSUMPHOSTAPPSVC_SUCCESS) {
        return false;
      }                                /* if */
    }
...
static void mdlInitializeSizes(SimStruct *S)
{
...

  {
    ...
    if (error != NULL) {
      ...
      ssSetErrorStatus( S,
                       "Failed to determine the installed In-the-Loop version for comparison against the In-the-Loop s-function version (release 9.4 (R2018a)_13). To avoid this error, remove the In-the-Loop s-function from your MATLAB path (e.g. delete it or move to a clean working directory).");
      return;
    }                                  /* if */
    ...
    if (strcmp(installVersion, "9.4 (R2018a)_13") != 0) {
      ssSetErrorStatus( S,
                       "The In-the-Loop s-function is incompatible with the installed In-the-Loop version (see ver('matlab')); it was generated for release 9.4 (R2018a)_13. To avoid this error, remove the In-the-Loop s-function from your MATLAB path (e.g. delete it or move to a clean working directory)");
      return;
    }                                  /* if */
